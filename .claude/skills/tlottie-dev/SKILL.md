...
12. **`package.json`'s `"bin"` field rejects a leading `./` on the path — npm silently drops the entire entry, not just that one path, and only warns during the actual `npm publish` run (`npm pack --dry-run` doesn't catch it either without reading the notice output carefully).** `"tlottie-outline": "./dist/bin/lottie-to-outline.js"` published three versions (0.1.2–0.1.4) with **no `tlottie-outline` binary at all** — nobody could `bunx`/`npx` it, and nothing in typecheck/lint/build caught it since the file itself builds fine; only the metadata around it was wrong. `main`/`module`/`exports` *do* want the `./` prefix, `bin` doesn't — don't assume the convention is uniform across package.json fields. `npm pkg fix` will tell you the corrected form if in doubt; run it (or at least `npm pack --dry-run` and actually read the notice/warn lines, not just skim for errors) before trusting a fresh `bin` entry.
